...
<html lang="en">
    ...
    <body class="container" >
        <form action="/quotes/">
            <button   style="float: right; outline: none; border: none; margin-top: 20px;margin-right:20px;" type="submit"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-x-square" viewBox="0 0 16 16">
        ...
        <div class="container" style="background-color: white; padding: 30px 30px;">
            ...
            <span><i>ĐT:0354072542</i></span>
            <h1 style="text-align: center; margin-top: 50px;margin-bottom: 20px;">QUOTES SLIP</h1>
            <span style="font-size: 20px;"><b>Code: ${quotes.getId()}</b></span>
            <span style="float: right;font-size: 20px"><b>Ngày:${quotes.getDate()}</b></span><br>
            <span style="font-size: 20px"><b>Customer:  <b>${customer.getName()}</b></b><i> (Schneider)</i></span><br>
            <span style="font-size: 20px;"><b>Phone: ${quotes.getPhone()} - Address: ${quotes.getAddress()}</b></span>
            <%!int i=0;%>
            <table style="width: 100%;margin-bottom: 30px;text-align: center" border="1px">
              ...
              <tr>   
                        ...
                        <td hidden="true"><input readonly style="border: none;outline: none;font-weight: bold" id="GP<%=i%>" value="${item.getGoodsPrice()}"/></td>
                        <td>${item.getDiscount()}</td>
                        <td><input readonly style="border: none;outline: none;width: 80px;" id="LastedPrice<%=i%>" value="${item.getLastedPrice()}"/></td>
            ...
            <span style="float:right;font-size: 25px;margin-right: 30px;"><b>Creator</b></span>
            <span style="font-size: 20px"><b>Total: 
             <input readonly style="border: none;outline: none;width: 70px;font-weight: bold" id="ALastedPrice" value=""/>
              
                </b></span><br><br>
            <span style="float:right;font-size: 20px;margin-right: 65px;"><i>(sign, full name)</i></span><br><br><br><br>
            <span style="float:right;font-size: 22px;margin-right: 70px;"><b>Admin</b></span>
            ...
            document.getElementById('TotalDiscount').value = nf.format(${quotes.getTotalDiscount()});
            document.getElementById('TatolPrice').value = nf.format(${quotes.getTatolPrice()});
            document.getElementById('ATD').value = nf.format(${quotes.getTotalDiscount()});
            document.getElementById('ATP').value = nf.format(${quotes.getTatolPrice()});
            document.getElementById('ALastedPrice').value = nf.format(${quotes.getLastedPrice()});
            ...
            document.getElementById(Afterpirce +count).value = nf.format(document.getElementById(GP + count).value);
